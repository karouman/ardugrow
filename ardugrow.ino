...
extern "C" const int _PIN_MOISTURE_[] = {     A5,      A7 };
extern "C" const int _PIN_WATER_PUMP_[] = {    3,       4 };
...
Relay waterPump( _PIN_WATER_PUMP_[ 0 ] );
...
void setup() {
  ...
  led.init();
  powerControl.init();
  pinMode( _PIN_WATER_LEVEL_, INPUT );
  pinMode( _PIN_WATER_LEVEL_GND_, OUTPUT );
  analogWrite( _PIN_WATER_LEVEL_GND_, LOW );
...
void loop() {
  ...
  int err = 0;
  if ( ( err = getTemperature( &temp, true ) ) != READ_OK ) {
    Serial.print( err );
    Serial.println( F( ": Cannot get temperature. You should check sensor connection." ) );
    temp = -1;
  }

  int nb_plants = sizeof( _PLANTS_ ) / sizeof( _PLANTS_[ 0 ] );
  Serial.print( F( "Size of _PLANTS_: " ) );
  Serial.println( nb_plants );
  ...
  for ( int i = 0; i < nb_plants; i++ ) {
    ...
    waterPump = Relay( _PIN_WATER_PUMP_[ i ] );
    waterPump.init();
    ...
    bool waterLevelCritic = ( analogRead( _PIN_WATER_LEVEL_ ) > 200 );
    Serial.print( "WaterLevel: " );
    Serial.println( analogRead( _PIN_WATER_LEVEL_ ) );
    if ( waterLevelCritic ) {
      Serial.println( "WARNING: Water Level is critic. Pumps are disabled." );
    }
    ...
    if ( wifi.connect( _WIFI_AP_SSID_, _WIFI_AP_PASSWD_ ) ) {
      ...
      if ( waterPumpSecondes > 0 && !waterLevelCritic ) {
        waterPump.on();
