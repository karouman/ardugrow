...
app.post("/", (req, res) => {
  ...
  // Algo
  // 1 - get plant trigger
  // 2 - read latest X records
  // 3 - forEach records if moisture is < trigger.minMoisture --> Plant has to be watered.
  // 4 - If plant has to be watered --> res.send( plant.trigger.water );
});
            ...
            ardugrowDb
              ...
              .toArray((err, result) => {
                if (err) {
                  ...
                  if (result.length === 0) throw ("No records", plant);

                  result.forEach(item => {
                    delete item.p;
                    delete item._id;
                    item.timeFormat = moment(item.dateTime).format(
                      "MM/DD/YYYY"
                    );
                    item.m = 1024 - item.m;
                  });
                  ...
                  resolve({
                    name: plant.name,
                    moment: moment(
                      result[result.length - 1].dateTime
                    ).calendar(),
